...
class ModelManager:
  def __init__(self):
    self.available_models = (params.get("AvailableModels") or "").split(",")
    self.model_versions = (params.get("ModelVersions") or "").split(",")
    self.model_series = (params.get("AvailableModelSeries") or "").split(",")
  ...
  def check_models(self, boot_run, repo_url):
    ...
    if params.get("Model") not in self.available_models:
      params.put("Model", params_default.get("Model"))
  ...
  def update_model_params(self, model_info, repo_url):
      ...
      try:
          if not params.get("ModelVersion"):
              default_model = params.get("Model") or DEFAULT_MODEL
