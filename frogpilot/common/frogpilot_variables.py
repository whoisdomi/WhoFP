...
frogpilot_default_params: list[tuple[str, str | bytes, int]] = [
  ...
  ("BlindSpotPath", "1", 1),
  ("BorderMetrics", "0", 3),
  ("CalibratedLateralAcceleration", str(DEFAULT_LATERAL_ACCELERATION), 2),
...
class FrogPilotVariables:
  ...
  def update(self, holiday_theme, started, boot_run=False):
    ...
    toggle.automatic_updates = params.get_bool("AutomaticUpdates") if tuning_level >= level["AutomaticUpdates"] and (self.release_branch or self.vetting_branch) else default.get_bool("AutomaticUpdates")
    ...
    toggle.no_logging = device_management and (params.get_bool("NoLogging") if tuning_level >= level["NoLogging"] else default.get_bool("NoLogging")) and not self.vetting_branch or params_memory.get_bool("ForceOnroad")
