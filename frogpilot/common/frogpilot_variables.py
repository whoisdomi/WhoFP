...
MAX_ACCELERATION = 10.0                    # ISO 15622:2018
...
DEVELOPER_SIDEBAR_METRICS = {
  ...
  "LONGITUDINAL_MPC_JERK_SPEED": 16,
  "MODEL_NAME": 17,
}
...
class FrogPilotVariables:
  ...
  def update(self, holiday_theme="stock", started=False):
    ...
    toggle.steerKf = self.get_value("SteerKf", cast=float, condition=advanced_lateral_tuning and is_torque_car, default=1.0, min=0.5, max=1.5)
    ...
    toggle.lowPassFilterAlpha = self.get_value("LowPassFilterAlpha", cast=float, condition=advanced_lateral_tuning, default=0.0, min=0, max=0.30)
    ...
    toggle.automatic_updates = self.get_value("AutomaticUpdates", default=True) and not BACKUP_PATH.is_file()
