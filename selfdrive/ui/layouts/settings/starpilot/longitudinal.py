    ...
    sections: list[SettingSection] = [
      SettingSection(tr_noop("Curve Speed Controller"), [
        ...
        SettingRow("ShowCSCStatus", "toggle", tr_noop("Status Widget"),
                   ...
                   visible=csc_on),
        SettingRow("CSCOffset", "value", tr_noop("CSC Offset"),
                   subtitle=tr_noop("Add or subtract from your learned lateral acceleration. Increase to take curves faster; decrease for more caution. 0.0 uses the calibrated value as-is."),
                   get_value=lambda: f"{self._params.get_float('CSCLateralAccelerationOffset'):.1f} m/s²",
                   on_click=lambda: self._show_slider("CSCLateralAccelerationOffset", -1.0, 1.0, step=0.1, unit=" m/s²", value_type="float"),
                   visible=csc_on),
