  ...
  async function selectSuggestion(sugg) {
    ...
    if (state.searchProvider === 'mapbox') {
      if (sugg.geometry && Array.isArray(sugg.geometry.coordinates)) {
        ...
        const url = new URL(
          `https://api.mapbox.com/search/searchbox/v1/retrieve/${encodeURIComponent(sugg.mapbox_id)}`
        )
                      ...
                      if (state.selectedRoute) {
                        return NavigationDestination({
                          ...
                          onConfirm: () => {
                            ...
                            state.confirmedRouteRefresh = Math.random()  // This line must be reactive
                          },
                          loadFavorites: loadFavoritesAlphabetically,
                          removeFavorite
                        }, state.confirmedRouteRefresh) // <- trigger re-render on this change
