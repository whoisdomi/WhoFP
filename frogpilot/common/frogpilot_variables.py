...
frogpilot_default_params: list[tuple[str, str | bytes, int, str]] = [
  ...
  ("FPSCounter", "1", 3, "0"),
  ("FrogPilotDongleId", "", 0, ""),
  ("FrogPilotDrives", "0", 0, "0"),
  ("FrogPilotKilometers", "0", 0, "0"),
  ("FrogPilotMinutes", "0", 0, "0"),
  ("FrogPilotStats", "", 0, ""),
...
class FrogPilotVariables:
  ...
  def update(self, holiday_theme, started, boot_run=False):
    ...
    if not is_torque_car:
      CarInterfaceBase.configure_torque_tune("MOCK", FPCP.lateralTuning)
    ...
    toggle.available_models = params.get("AvailableModels", encoding="utf-8") + f",{DEFAULT_MODEL}"
    toggle.available_model_names = params.get("AvailableModelNames", encoding="utf-8") + f",{DEFAULT_MODEL_NAME}"
    ...
    model_versions = params.get("ModelVersions", encoding="utf-8") + f",{DEFAULT_MODEL_VERSION}"
