...
FrogPilotLateralPanel::FrogPilotLateralPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  for (const auto &[param, title, desc, icon] : lateralToggles) {
    ...
    if (param == "AdvancedLateralTune") {
      ...
      lateralToggle = new FrogPilotParamValueButtonControl(param, title, desc, icon, 0.01, 1.0, QString(), std::map<float, QString>(), 0.01, false, {}, steerDelayButton, false, false);
...
void FrogPilotLateralPanel::showEvent(QShowEvent *event) {
  ...
  steerDelayToggle->updateControl(delayStock * 0.5, delayStock * 5);
  ...
  steerKPToggle->updateControl(kpStock * 0.25, kpStock * 1.5);
