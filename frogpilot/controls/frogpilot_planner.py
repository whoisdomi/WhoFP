...
class FrogPilotPlanner:
  ...
  def update(self, now, time_validated, sm, frogpilot_toggles):
    ...
    predicted_lateral_acceleration = abs(self.road_curvature * max(v_ego, 1)**2)

    if self.road_curvature_detected:
      curvature_detected = predicted_lateral_acceleration > 0.9
    else:
      curvature_detected = predicted_lateral_acceleration > 1.1

    self.road_curvature_detected = curvature_detected and v_ego > CRUISING_SPEED and not (sm["carState"].leftBlinker or sm["carState"].rightBlinker)
    ...
    if frogpilot_toggles.icbm_enabled:
      ...
      if icbm_button != self._prev_icbm_button:
        self.params_memory.put("ICBMButton", icbm_button)
        self._prev_icbm_button = icbm_button
  ...
  def publish(self, theme_updated, toggles_updated, sm, pm, frogpilot_toggles):
    ...
    frogpilotPlan.redLight = self.frogpilot_cem.stop_light_detected or sm["frogpilotCarState"].dashboardStopSign
