...
class FrogPilotTracking:
  def __init__(self, frogpilot_planner, frogpilot_toggles):
    ...
    self.frogpilot_stats.setdefault("ExperimentalModeTime", self.frogpilot_stats.get("ExperimentalMode", 0))
    self.frogpilot_stats.setdefault("FrogPilotDrives", params.get_int("FrogPilotDrives"))
    self.frogpilot_stats.setdefault("FrogPilotKilometers", params.get_float("FrogPilotKilometers"))
    self.frogpilot_stats.setdefault("FrogPilotMeters", self.frogpilot_stats.get("FrogPilotKilometers", 0) * 1000)
    self.frogpilot_stats.setdefault("FrogPilotMinutes", params.get_float("FrogPilotMinutes"))
    self.frogpilot_stats.setdefault("FrogPilotSeconds", self.frogpilot_stats.get("FrogPilotMinutes", 0) * 60)
    ...
    self.frogpilot_stats = {key: value for key, value in self.frogpilot_stats.items() if key not in ("ExperimentalMode", "FrogPilotKilometers", "FrogPilotMinutes")}

    model_times = self.frogpilot_stats.get("ModelTimes", {})
    if isinstance(model_times, dict):
      cleaned_model_times = {}
      for name, seconds in model_times.items():
        cleaned_name = clean_model_name(name)
        cleaned_model_times[cleaned_name] = cleaned_model_times.get(cleaned_name, 0) + float(seconds)

      self.frogpilot_stats["ModelTimes"] = cleaned_model_times
